...
@app.route('/<category>')
def hwy(category):
    categoryItem = session.query(Catalog).filter_by(name = category).first()
    print "", categoryItem.id, categoryItem.name
    return "hey %s" % categoryItem.name
...
def shoCategory():
    ...
    # .filter_by(id = catalog.id)
    # return render_template('publicItemList.html', items=items)
    items = session.query(CatalogList).all()
    if 'username' not in login_session:
        print "hi", login_session
        return render_template('publicItemList.html', categories=categories, items=items)
    else:
        print "hi", login_session
...
def editCategory(id):
    ...
    if request.method == 'POST':
        ...
        print "Edit name: ", editCategory.name
        return redirect(url_for('shoCategory'))
    return render_template('editCategoryName.html',editCategory=editCategory, id=id)
    # return "This page will be for editing restaurant %s" % catalog_id
...
def deleteCategory(id):
    ...
    if request.method == 'POST':
        ...
        session.commit()
        return redirect(url_for('shoCategory', id = id))
    return render_template('delteCategoryName.html', categoryItem=categoryItem, id=id)
...
def showItems(id):
    ...
    if 'username' not in login_session or creator.id != login_session['user_id']:
        return render_template('publicShowItems.html', categories=categories, items=items, id=id, creator=creator)
    else:
        return render_template('showItems.html', categories=categories, items=items, id=id, creator=creator)

@app.route('/catalog/<int:id>/items/new', methods=['GET', 'POST'])
@loggedIn
def newMenuItem(id):
    if request.method == 'POST':
        newItem = CatalogList(name = request.form['name'], description = request.form['description'], menu_id=id)
        session.add(newItem)
        session.commit()
        print "session: ", newItem.id , newItem.name, newItem.menu_id
        return redirect(url_for('showItems', id=id))
...
@app.route('/catalog/<int:id>/<int:item_id>/edit', methods=['GET', 'POST'])
@loggedIn
def editMenuItem(id, item_id):
    # catalog = session.query(Catalog).filter_by(id = id).one()
    # items = session.query(Catalog).all()
    categories = session.query(Catalog).filter_by(id = id).one()#.all()
    editItem = session.query(CatalogList).filter_by(id = item_id).one()
    if request.method == 'POST':
        editItemName = CatalogList(name = request.form['editname'], description = request.form['editdescription'], catalog = request.form['editcategory'])
        editItem.name = editItemName.name
        session.add(editItem)
        session.commit()
        return redirect(url_for('showItems', id=id))
    return render_template('editItemName.html', id=id, item_id=item_id, editItem=editItem)
    # return "This page is for editing menu item %s" % item_id

@app.route('/catalog/<int:id>/<int:item_id>/delete', methods=['GET', 'POST'])
@loggedIn
def deleteMenuItem(id, item_id):
    items = session.query(Catalog).all()
    deleteItem = session.query(CatalogList).filter_by(id = item_id).one()
    if request.method == 'POST':
        ...
        session.commit()
        return redirect(url_for('showItems', id=id))
    return render_template('deleteItemName.html', id=id, item_id=item_id, deleteItem=deleteItem)
...
def showItemDetail(id, item_id):
    ...
    items = session.query(CatalogList).filter_by(id = item_id).one()
    if 'username' not in login_session or creator.id != login_session['user_id']:
        return render_template('publicShowItemDetail.html', id=id, item_id=item_id, items=items, categories=categories)
    else:
        return render_template('showItemDetail.html', id=id, item_id=item_id, items=items, categories=categories)
    # return "This page is the menu for restaurant %s" % id
